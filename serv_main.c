...
/* default to using the WMO feedtype */
...
int main (int argc, char *argv[])
{
	...
	if (!ServOpt.debug && ServOpt.daemon) {
		daemonize();

#ifdef AWC_LDM_SUPPORT
		if (getenv("LDMHOME")) {
			sprintf(pidfile, "%s/etc/%s-%s-%d", getenv("LDMHOME"), ServOpt.hostname, Program, ServOpt.listen_port);
		} else {
#endif
...
GLOBAL VARIABLES (from ServOpt structure)
	...
	char			daemon			O	allow program to run as a daemon
#ifdef AWC_LDM_SUPPORT
	char *			ldm_output		O	send output to LDM, not files
	char			ldm_addpil		O	include AWIPS id in LDM identifier
	char *			ldm_queue		O	LDM queue to write to
	int				ldm_feedtype	O	LDM feedtype to assign to products
	unsigned int	ldm_products	O	LDM product counter
	unsigned int	ldm_dups    	O	LDM duplicate product counter
	pqueue *		ldm_pq      	O	full path to the LDM product queue
	MD5_CTX *		ldm_md5context  O	LDM MD5 checksum context
	char *			hostname		O	cached local hostname
#endif
...
static void process_args(int argc, char *argv[])
{
	...
	ServOpt.ldm_output = 0;
	ServOpt.ldm_addpil = 0;
	ServOpt.ldm_dups = 0;
	ServOpt.ldm_products = 0;
    strcpy(ServOpt.ldm_queue,getenv("HOME"));
    strcat(ServOpt.ldm_queue,"/data/ldm.pq");
    ServOpt.ldm_feedtype = DEFAULT_FEEDTYPE;
	gethostname(ServOpt.hostname, sizeof(ServOpt.hostname));
#endif
	ServOpt.verbosity = 0;
	...
	while ((c = getopt(argc, argv, "dBLAv:ap:w:t:b:c:l:D:OPm:Q:F:s:")) != -1) {
		switch (c) {
			case 'd':
...
#ifdef AWC_LDM_SUPPORT
			case 'L':
				fprintf(stdout, "%s: Setting LDM output option\n", Program);
				ServOpt.ldm_output = 1;
				break;
			case 'A':
				fprintf(stdout, "%s: Setting AWIPS pil id LDM option\n", Program);
				ServOpt.ldm_addpil = 1;
				break;
			case 'Q':
				fprintf(stdout, "%s: Setting LDM queue to %s\n",
						Program, optarg);
				...
				strcpy(ServOpt.ldm_queue, optarg);
				break;
			case 'F':
				fprintf(stdout, "%s: Setting LDM feedtype to %s\n",
						Program, optarg);
				ServOpt.ldm_feedtype = atofeedtypet(optarg);
				if (ServOpt.ldm_feedtype == NONE) {
					fprintf(stderr,
						"%s: ERROR Invalid LDM feedtype %s\n",
						Program, optarg);
					exit(1);
				}
				break;
#endif
			case 'B':
				fprintf(stdout, "%s: Disabling background daemon process\n", Program);
				ServOpt.daemon = 0;
				break;
			case 'l':
...
#ifdef AWC_LDM_SUPPORT
	fprintf(stderr,
		"         [-L]             (send product to LDM instead of files, default NO)\n");
	fprintf(stderr,
		"         [-A]             (include AWIPS identifier after WMO header in LDM id, default NO)\n");
	fprintf(stderr,
		"         [-Q path]        (full path LDM queue, default $HOME/data/ldm.pq)\n");
	fprintf(stderr,
		"         [-F feedtype]    (LDM feedtype, default %s)\n", s_feedtypet(DEFAULT_FEEDTYPE));
#endif
	fprintf(stderr,
		"         [-B]             (disable running as a background daemon process)\n");
	fprintf(stderr,
