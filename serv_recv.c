...
static int recv_prod(int sock_fd, char *recvbuf, size_t bufsiz, prod_info_t *p_prod)
{
    ...
    if (!ServOpt.ldm_output) {
        ...
        /* daryl herzmann edits (2 Mar 2011)
 	 *  We know that the product terminates with \003 from above, now 
 	 *  lets get rid of any meaningless \r\r\n before this \003, leaving
 	 *  only one \r\r\n before the final \003 
 	 */
        int spos;
